...
| Volume | `{{PROJECT_NAME}}-data` | `backups/volume_{{PROJECT_NAME}}_YYYYMMDD_HHMMSS.tar.gz` |
...
# Volume 恢复
docker run --rm -v {{PROJECT_NAME}}-data:/target -v $(pwd)/backups:/backup \
    alpine tar xzf /backup/volume_{{PROJECT_NAME}}_XXXXXXXX.tar.gz -C /target
```
